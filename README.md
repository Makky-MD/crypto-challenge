...
- `probability.py`
  修改了部分参数
- `4.py`
  一个可以自动提取当前路径下的文件名中的"序号" "文件名""输入掩码u""输出掩码v""活跃S盒数"信息的脚本
- `5.py`
  一个可以自动修改前面文件参数以及依次运行前面文件的脚本
- `computercor.exe`
  由`computercor.cpp`编译的可执行文件
- `6.py`
  一个可以自动调用`computercor.exe`验证结果的脚本
```
初版贴合题目的代码第一次修改/
├── 2026密码数学挑战赛-赛题三.md    # 赛题说明文档
├── probability.py                    # 步骤1: 搜索差分概率边界
├── 3_fix r&p - 2.py                 # 步骤2: SAT求解器（搜索差分特征）
├── 3_recover_path - 2.py            # 步骤3: 解析SAT结果，恢复差分路径
├── 4.py                             # 步骤4: 汇总路径信息到CSV
├── 5.py                             # 自动化测试脚本（整合步骤1-4）
├── 6.py                             # 步骤5: 调用C++程序验证结果
├── computecor.cpp                   # C++暴力计算相关度源文件
└── computecor.exe                   # C++程序编译后的可执行文件
```

### 9. 稀疏矩阵初版代码/
- 稀疏矩阵-一轮-所有一轮绝对值最大路径（288条）.py
  功能 ：搜索一轮SPN加密中相关系数绝对值为0.5的所有路径（共288条）
- 稀疏矩阵-一轮-给定输入（一个活跃S盒）搜索.py
  功能 ：给定特定输入掩码，搜索一轮加密的所有路径
- 稀疏矩阵-两轮-2.py
  功能 ：两轮SPN线性密码分析的核心实现（带详细注释）
- 稀疏矩阵-两轮-2-给定输入输出，得到所有路径.py
  功能 ：给定输入和输出掩码，计算两轮加密的线性相关度并找出所有路径
- 稀疏矩阵-满足条件的二轮相关度-搜索(1).py
  功能 ：批量搜索所有单活性S盒输入掩码，筛选满足相关度阈值的结果并导出Excel
- linear_cryptanalysis_results-2轮.xlsx
  功能 ：已生成的分析结果Excel文件
```
稀疏矩阵初版代码/
├── 稀疏矩阵-一轮-所有一轮绝对值最大路径（288条）.py
