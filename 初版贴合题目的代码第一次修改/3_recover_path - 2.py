...
def main():
    """主函数：自动检测并处理所有 solution_*.txt 文件"""
    import glob
    
    # 找到所有 solution_*.txt 文件
    files = glob.glob('solution_*.txt')
    
    if not files:
        print("没有找到 solution_*.txt 文件")
        return
    
    print(f"找到 {len(files)} 个 solution 文件")
    
    # 提取轮数和概率信息
    patterns = set()
    for f in files:
        round_match = re.search(r'r(\d+)', f)
        prob_match = re.search(r'p(\d+)', f)
        if round_match and prob_match:
            patterns.add(f"solution_r{round_match.group(1)}_p{prob_match.group(1)}_*.txt")
    
    # 处理每个模式
    for pattern in patterns:
        print(f"\n处理模式: {pattern}")
        batch_process_solutions(pattern)
